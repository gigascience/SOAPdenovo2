...
file_t* load_file_lines(FILE *in, int block_rows, int *line_breaker_table, int *col_breaker_table,
						int *col_idxs, int *col_starts, int *col_ends, int *col_types, hashci_t **enums, int *orders, int *str_cases, int col_size){
	file_t *db;
	line_t *line;
	sortkey_t *key;
...
#define BUFFER_SIZE (32 * 1024)
	...
	while((n = fread(buffer, 1, BUFFER_SIZE, in)) > 0){
		if(db->file_size + n >= db->file_cap){
			db->file_cap = (size_t)(db->file_size * 1.2) + n;
			db->text = (char*)realloc(db->text, db->file_cap + 1);
		}
...
void usage(char *prog){
	printf(
	"msort, sort file rows by multiple field, wriiten by  Ruan Jue <ruanjue@genomics.org.cn>\n"
	"Usage: %s [-hrfn] [-t <filed_separators>]  [-k [rfmne]<col>[start-end]{enum1,...},...] [<file>]\n"
	"Options:\n"
...
int main(int argc, char **argv){
	...
	block_rows = 1;
	regcomp(&regex, "([rnmfe]*)([0-9]+)(\\[([0-9]+)(-([0-9]+)?)?\\])?(\\{([^}]+)\\})?", REG_EXTENDED);
	while((c = getopt(argc, argv, "hrmnfdt:l:L:k:")) != -1){
		switch(c){
			case 'h':
				usage(argv[0]);
				break;
	...
	if(db == NULL){
		fprintf(stderr, "Please report below message to me <ruanjue@genomics.org.cn>\n");
