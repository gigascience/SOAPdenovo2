...
protected:
	...
	ifstream* finp;  //single input file
...
public:
	...
	ReadHash(ifstream* _finp, LibInfo* _pLibInfo=NULL, Short_Len_t _overlapParam=25, Number_t _readsSum=0, Number_t _hashLen=SEED_NUM, float _loadFactor=0.75) : 
		finp(_finp), 
		pLibInfo(_pLibInfo), 
		reads(NULL), 
		readsSum(_readsSum), 
	...
	void initialize() {
		...
		cout << "counting reads" << endl;
		start_time=time(NULL);
		
		countReads();
		cout << endl;
		cout << "reads sum: " << readsSum <<endl;
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		cout << "reads counted" << endl;
		cout << endl;
		
		if (readsSum > 4294967296)
		{
			cout << "At most 4294967296 reads are supported.\nPlease use less reads and try again.\n";
			exit (1);
		}
		...
		cout << "initializing reads" << endl;
		start_time=time(NULL);
		
		initializeReads();
		
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		cout << "reads initialized" << endl;
		cout << endl;
		
		
		
		cout << "sorting reads in reversed direction" << endl;
		start_time=time(NULL);
		
		sortReadsReverse();
		
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		cout << "reads sorted" << endl;
		cout << endl;
		
		
		
		cout << "initializing hash on reversed reads" << endl;
		start_time=time(NULL);
		
		initializeReadsHashReverse();
		
		cout << "kmers sum: " << getKmersSum() <<endl;
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		...
		cout << "reversing reads" << endl;
		start_time=time(NULL);
		
		reverseReads();
		
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		cout << "reads reversed" << endl;
		cout << endl;
		
		
		
		cout << "sorting reads in forward direction" << endl;
		start_time=time(NULL);
		...
		cout << "initializing hash on forward reads" << endl;
		start_time=time(NULL);
		...
		cout << "marking unique reads" << endl;
		start_time=time(NULL);
		
		uniqueReads();
		
		cout << "spent time: " << time(NULL)-start_time << "s" <<endl;
		cout << "unique reads marked" << endl;
		cout << endl;
		
		
		
		cout << "sorting reads by serial number" << endl;
		start_time=time(NULL);
		...
		cout << ">>>>>>>>>>hash initialization finished<<<<<<<<<<" << endl;
		cout << endl;
	}
...
private:
	...
	void countReads() {
		...
		if (pLibInfo) {
			...
			for (i=0; i<(int)pLibInfo->getArrayLen(); i++) {
				...
				if (libInfo.readsSum==0 || libInfo.pairReadsSum==0) {
					...
					if (libInfo.readsSum==0 && libInfo.pairReadsSum==0) {
						...
						for(j=0;j<libInfo.num_p_file;j++) {
							if (libInfo.p_fname[j].substr(libInfo.p_fname[j].size()-3) != ".gz"){
								...
								if(!_fin) {
									cout << endl << "Error: can not open " << libInfo.p_fname[j] << endl;
									exit(1);
								}
						...
						for(j=0;j<libInfo.num_a1_file;j++) {
							if (libInfo.a1_fname[j].substr(libInfo.a1_fname[j].size()-3) != ".gz"){
								...
								if(!_fin) {
									cout << endl << "Error: can not open " << libInfo.a1_fname[j] << endl;
									exit(1);
								}
								libInfo.pairReadsSum += countReadsFa(_fin);
								_fin.close();
						...
						for(j=0;j<libInfo.num_a2_file;j++) {
							if (libInfo.a2_fname[j].substr(libInfo.a2_fname[j].size()-3) != ".gz"){
								...
								if(!_fin) {
									cout << endl << "Error: can not open " << libInfo.a2_fname[j] << endl;
									exit(1);
								}
								libInfo.pairReadsSum += countReadsFa(_fin);
								_fin.close();
						...
						for(j=0;j<libInfo.num_q1_file;j++) {
							if (libInfo.q1_fname[j].substr(libInfo.q1_fname[j].size()-3) != ".gz"){
								...
								if(!_fin) {
									cout << endl << "Error: can not open " << libInfo.q1_fname[j] << endl;
									exit(1);
								}
					...
					for(j=0;j<libInfo.num_s_a_file;j++) {
						if (libInfo.s_a_fname[j].substr(libInfo.s_a_fname[j].size()-3) != ".gz"){
							...
							if(!_fin) {
								cout << endl << "Error: can not open " << libInfo.s_a_fname[j] << endl;
								exit(1);
							}
				...
				cout << "    total reads sum:     " << libInfo.readsSum << endl;
				cout <<"    Paired-end reads sum: " << libInfo.pairReadsSum <<endl;
			}
		}
		else {
	...
	Number_t countReadsFa(ifstream& fin) {
	...
	Number_t countReadsFaGZ(string& fin) {
	...
	Number_t countReadsFq(ifstream& fin) {
	...
	Number_t countReadsFqGZ(string& fin) {
	...
	void initializeReadsFa(ifstream& fin) {
...
//		fin.seekg(0, ios_base::beg);
	...
	void initializeReadsFaGZ(string& fin) {
	...
	void initializeReadsFaPair(ifstream& fin1, ifstream& fin2) {
	...
	void initializeReadsFaPairGZ(string& fin1, string& fin2) {
	...
	void _initializeReads(char* str, size_t size) {
		...
		if (compare(numbers, size, numbersRev, size) > 0) {
			reads[readsSum].initialize(numbers, size);
		}
		else {
			reads[readsSum].initialize(numbersRev, size);
		}
